...
function WeightChange(a){
  ...
  async function loadTables1(tableid,q){
    resetTables(tableid);
    console.log("Load");
    ...
    querySnapshot.forEach((doc) => {
        ...
        var download = doc.data().파일;
        console.log("File "+id+": " +title,body,date,download);
        AddItemsToTable1(tableid,num,title,date,download);
    });
  }

  loadTables1('tbody1',q1);


  function AddItemsToTable1(tableid,num,title,date,download){
...
async function loadTables2(tableid,q){
    resetTables(tableid)
    console.log("Load");
    const querySnapshot = await getDocs(q);
    var count = querySnapshot.docs.length;
    querySnapshot.forEach((doc) => {
        ...
        var download = doc.data().파일;
        console.log("File "+id+": " +title,body,date,download);
        AddItemsToTable2(tableid,num,title,date,download);
    });
  }

function AddItemsToTable2(tableid,num,title,date,download){
    var tbody = document.getElementById(tableid);
    var trow = document.createElement('tr');
    var td1 = document.createElement('td');
    var td2 = document.createElement('td');
